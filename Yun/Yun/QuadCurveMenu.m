...
#define ENDRADIUS 135.0f
#define FARRADIUS 160.0f
#define STARTPOINT CGPointMake(320.f/2, [UIScreen mainScreen].bounds.size.height - 40.f)
...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray
{
    ...
    if (self) {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            if (i < 2)
            {
                xEnd = STARTPOINT.x - ENDRADIUS * cosf(i * M_PI / (count - 1));
                yEnd = STARTPOINT.y - ENDRADIUS * sinf(i * M_PI / (count - 1));
                
                xNear = STARTPOINT.x - NEARRADIUS * cosf(i * M_PI / (count - 1));
                yNear = STARTPOINT.y - NEARRADIUS * sinf(i * M_PI / (count - 1));
                
                xFar = STARTPOINT.x - FARRADIUS * cosf(i * M_PI / (count - 1));
                yFar = STARTPOINT.y - FARRADIUS * sinf(i * M_PI / (count - 1));
            }
            else
            {
                xEnd = STARTPOINT.x + ENDRADIUS * cosf(M_PI - i * M_PI / (count - 1));
                yEnd = STARTPOINT.y - ENDRADIUS * sinf(M_PI - i * M_PI / (count - 1));
                
                xNear = STARTPOINT.x + NEARRADIUS * cosf(M_PI - i * M_PI / (count - 1));
                yNear = STARTPOINT.y - NEARRADIUS * sinf(M_PI - i * M_PI / (count - 1));
                
                xFar = STARTPOINT.x + FARRADIUS * cosf(M_PI - i * M_PI / (count - 1));
                yFar = STARTPOINT.y - FARRADIUS * sinf(M_PI - i * M_PI / (count - 1));
            }
            
            
            item.endPoint = CGPointMake(xEnd, yEnd);
            
            item.nearPoint = CGPointMake(xNear, yNear);
            item.farPoint = CGPointMake(xFar, yFar);
        ...
        // add the "Add" Button.
        _addButton = [[QuadCurveMenuItem alloc] initWithImage:[UIImage imageNamed:@"home_cloud.png"]
                                       highlightedImage:[UIImage imageNamed:@"home_cloud.png"]
